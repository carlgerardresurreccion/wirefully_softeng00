...
def create_phone_wireframe_image(layout, use_case):
    img_width = 400
    img_height = 800
    screen_margin = 20
    border_radius = 20
    border_width = 2
    screen_width = img_width - 2 * screen_margin
    screen_height = img_height - 2 * screen_margin - 100  # Adjust for phone frame

    img = Image.new('RGB', (img_width, img_height), 'white')
    draw = ImageDraw.Draw(img)

    # Draw phone frame
    phone_frame = round_rectangle((img_width - 40, img_height - 40), border_radius, 'white', 'black', border_width)
    img.paste(phone_frame, (20, 20), phone_frame)

    # Draw top speaker
    speaker_width = 60
    speaker_height = 10
    speaker_x = (img_width - speaker_width) // 2
    speaker_y = 40
    draw.rectangle([(speaker_x, speaker_y), (speaker_x + speaker_width, speaker_y + speaker_height)], fill='black')

    # Draw home button
    home_button_radius = 30
    home_button_x = (img_width - home_button_radius) // 2
    home_button_y = img_height - 60
    draw.ellipse([(home_button_x, home_button_y), (home_button_x + home_button_radius, home_button_y + home_button_radius)], outline='black', width=2)

    phone_screen = round_rectangle((screen_width, screen_height), border_radius, "white", "black", border_width)
    img.paste(phone_screen, (screen_margin, screen_margin + 50))

    #layout = generate_layout(use_case)

    #if use_case == "Dashboard":
    #    draw_dashboard_navigation_bar(draw, screen_margin, screen_width)
    #else:
    #    draw_navigation_bar(draw, screen_margin, screen_width)

    #current_y = screen_margin + 100  # Starting y-coordinate for content

    #for key, element in layout.items():
    #    element_width = element['width']
    #    element_height = element['height']
    #    element_x = screen_margin + (screen_width - element_width) // 2  # Center element horizontally
    #    draw_text_field(draw, (element_x, current_y), element_width, element_height, key, border_width=2, border_color='black')
    #    current_y += element_height + 20  # Move y-coordinate for next element

    img.save('phone_wireframe.png')
    img.show()